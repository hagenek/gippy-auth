...
export default function Home({ params: { slug } }) {
  ...
  useEffect(() => {
    ...
    axios
      .get("https://5bc9-46-212-186-13.ngrok-free.app/get_login_url", {
        params: { id: slug },
      })
      .then((res) => {
        setAuthUrl(res.data);
  ...
  return (
    <main className="flex min-h-screen w-full flex-col gap-3 items-start justify-start p-4 py-8 md:p-24 bg-slate-200">
      ...
      <button
        className="flex items-center justify-center py-4 bg-slate-700 px-4 py-3 text-white"
      ...
      >
        <Image
          ...
          height={20}
          src="/google-logo.svg"
          className=" me-2 p-1 bg-slate-50 rounded-full"
        />
        Login with Google
      </button>
    </main>
